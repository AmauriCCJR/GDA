<div class="container">
  <div class="row align-items-start gy-4">

    <div class="col-lg-3 col-md-6 col-12 text-start">
      <h5 class="gda_footer_title">Plataforma</h5>
      <ul class="list-unstyled gda_footer_li">
        <li><a href="#">Otimize sua Importação</a></li>
        <li><a href="#">Simplifique sua Exportação</a></li>
        <li><a href="#">Gestão Inteligente de Armazéns</a></li>
        <li><a href="#">Consultoria Aduaneira Especializada</a></li>
      </ul>
    </div>

    <div class="col-lg-3 col-md-6 col-12 text-start">
      <h5 class="gda_footer_title">Suporte</h5>
      <ul class="list-unstyled gda_footer_li">
        <li><a href="contatos.php">Contatos</a></li>
        <li><a href="duvidas_frequentes.php">Dúvidas Frequentes</a></li>
      </ul>
    </div>

    <div class="col-lg-3 col-md-6 col-12 text-start">
      <h5 class="gda_footer_title">Institucional</h5>
      <ul class="list-unstyled gda_footer_li">
        <li><a href="termos_de_uso.php">Termos de Uso</a></li>
        <li><a href="politica_de_privacidade.php">Política de Privacidade</a></li>
      </ul>
    </div>

    <div class="col-lg-3 col-md-6 col-12 d-flex flex-column align-items-lg-end align-items-center">
      <a href="index.php" class="mb-0">
        <img src="../assets/img/logo_footer2.png" alt="GDA" class="gda_footer_logo">
      </a>
      
      <div class="gda_footer_rede text-center p-3">
        <h5 class="gda_footer_title_rede mb-3">Nossas Redes</h5>
        <div class="d-flex justify-content-center gap-3">
          <a href="#" class="icon-footer"><img src="../assets/img/whatsapp.png" alt="WhatsApp"></a>
          <a href="#" class="icon-footer"><img src="../assets/img/instagram.png" alt="Instagram"></a>
          <a href="#" class="icon-footer"><img src="../assets/img/linkedin.png" alt="LinkedIn"></a>
          <a href="#" class="icon-footer"><img src="../assets/img/facebook.png" alt="Facebook"></a>
        </div>
      </div>
    </div>

  </div>
</div>

